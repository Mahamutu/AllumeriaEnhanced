...
void main()
{
    ...
    float dissolveMask = texture(texture2, texCoord).r;
    base.a -= dissolveMask * dissolve;
    if (base.a < 0.5)
        discard;

    vec3 effect = vec3(max(dissolveMask * dissolve * 2.0 - 0.5, 0.0) + hitFade * 0.5, 0.0, 0.0);
    vec3 emission = texture(texture1, texCoord).rgb;
    vec3 vanilla = base.rgb * vanillaVertexCol.rgb + emission + effect;
    ...
    vec3 shadowedLight = max(base.rgb * vertexCol.rgb - base.rgb * directSunlight * shadowAmount, vec3(0.0));
    vec3 toTorch = ae_handLightPosition - fragPosition;
    float torchDistance = max(length(toTorch), 0.45);
    float torchFacing = 0.35 + 0.65 * max(dot(normalize(worldNormal),
        toTorch / torchDistance), 0.0);
    vec3 torchLight = ae_handLightColor * min(5.5 / (torchDistance * torchDistance), 1.0)
        * torchFacing;
    // The held emitter is located inside its own model. Always apply its warm
    // self-light instead of relying on the cubemap visibility test there.
    if(ae_isHand>0.5 || ae_localActive<0.5)
        shadowedLight = max(shadowedLight, base.rgb * torchLight);
    shadowedLight += base.rgb * localDirectLight(fragPosition,worldNormal);
    vec3 enhanced = gradeColor(shadowedLight + effect) + emission;
    ...
    vec3 vanillaFogged = mix(atmosphere, vanilla, visibility);
    ...
    vec3 enhancedFogged = mix(atmosphericScattering(viewRay,d), enhanced, enhancedVisibility);
    outputColor = vec4(mix(vanillaFogged, enhancedFogged, clamp(ae_enabled, 0.0, 1.0)), base.a);
